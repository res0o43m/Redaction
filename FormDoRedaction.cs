...
namespace Redaction
{
    public partial class FormDoRedaction : Form
    {
        ...
        private Ribbon2 RedactRibbon;        
        private System.ComponentModel.BackgroundWorker backgroundWorkerRedact;

        public FormDoRedaction(Word.Document document, Ribbon2 ribbon)
        {
            InitializeComponent();
        ...
        internal void ResetProgress()
        {
            if (InvokeRequired)
            {
                Invoke(new ResetProgressDelegate(ResetProgress), Array.Empty<object>());
                return;
            }
        ...
        internal void CloseDialog()
        {
            if (InvokeRequired)
            {
                Invoke(new CloseDialogDelegate(CloseDialog), Array.Empty<object>());
                return;
            }
        ...
        internal void FillProgressBar()
        {
            if (InvokeRequired)
            {
                Invoke(new FillProgressBarDelegate(FillProgressBar), Array.Empty<object>());
                return;
            }
